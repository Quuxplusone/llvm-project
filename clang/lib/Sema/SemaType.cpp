  ...
  for (unsigned i = 0, e = D.getNumTypeObjects(); i != e; ++i) {
    ...
    case DeclaratorChunk::Function: {
      ...
      if ((T.getCVRQualifiers() || T->isAtomicType()) &&
        ...
        // type. Don't warn on undeduced auto types because then you'll get
        // duplicate warnings: one on the template and one on the instantiation.
        // For dependent types that aren't deduced, though, we must warn
        // here because it's the only place we'll ever see.
        if (S.getLangOpts().CPlusPlus &&
            (T->isRecordType() || T->isDependentType())) {
          // Diagnose with a different diagnostic.
          diagnoseRedundantReturnTypeQualifiers(
              diag::warn_qual_class_return_type, S, T, D, chunkIndex);
          ...
          diagnoseRedundantReturnTypeQualifiers(diag::warn_qual_return_type, S,
                                                T, D, chunkIndex);
      }
