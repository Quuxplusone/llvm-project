// RUN: %clang_cc1 -std=c++20 -verify -DTEST_LINUX %s
// RUN: %clang_cc1 -std=c++20 -verify -DTEST_DARWIN %s
// RUN: %clang_cc1 -std=c++20 -verify -DTEST_MSWIN %s
...
int val = square(2);
// expected-error@-1 {{call to consteval function 'square' is not a constant expression}}
