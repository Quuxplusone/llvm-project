...
void no_threeway_operator() {
  ...
  // expected-note@*:* {{and 'partial_order(std::forward<_Tp>(__t), std::forward<_Up>(__u))' would be invalid}}
  // expected-note@*:* {{and 'std::weak_order(std::forward<_Tp>(__t), std::forward<_Up>(__u))' would be invalid}}
}
...
void too_weak() {
  ...
  // expected-error@-1 {{no matching function for call}}
  // expected-note@*:* {{candidate template ignored: constraints not satisfied}}
  // expected-note@*:* {{because 'std::partial_ordering(std::forward<_Tp>(__t) <=> std::forward<_Up>(__u))' would be invalid}}
  // expected-note@*:* {{and 'partial_order(std::forward<_Tp>(__t), std::forward<_Up>(__u))' would be invalid}}
  // expected-note@*:* {{and 'std::weak_order(std::forward<_Tp>(__t), std::forward<_Up>(__u))' would be invalid}}
}
