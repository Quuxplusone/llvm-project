//===----------------------------------------------------------------------===//
//
//                     The LLVM Compiler Infrastructure
//
// This file is dual licensed under the MIT and the University of Illinois Open
// Source Licenses. See LICENSE.TXT for details.
//
//===----------------------------------------------------------------------===//

// UNSUPPORTED: c++98, c++03, c++11, c++14

// <memory>

// template <class BidirIt1, class BidirIt2>
//   BidirIt2 uninitialized_relocate_backward(BidirIt1, BidirIt1, BidirIt2);

#include <iterator>
#include <memory>
#include <cstdlib>
#include <cassert>

#include "test_macros.h"
#include "test_iterators.h"

struct Counted {
  static int count;
  static int constructed;
  static void reset() { count = constructed = 0; }
  explicit Counted(int&& x) : value(x) { x = 0; ++count; ++constructed; }
  Counted(Counted const&) { assert(false); }
  ~Counted() { assert(count > 0); --count; }
  friend void operator&(Counted) = delete;
  int value;
};
int Counted::count = 0;
int Counted::constructed = 0;

struct ThrowsCounted {
  static int count;
  static int constructed;
  static int throw_after;
  static void reset() { throw_after = count = constructed = 0; }
  explicit ThrowsCounted(int&& x) {
    ++constructed;
    if (throw_after > 0 && --throw_after == 0) {
      TEST_THROW(1);
    }
    ++count;
    x = 0;
  }
  ThrowsCounted(ThrowsCounted const&) { assert(false); }
  ~ThrowsCounted() { assert(count > 0); --count; }
  friend void operator&(ThrowsCounted) = delete;
};
int ThrowsCounted::count = 0;
int ThrowsCounted::constructed = 0;
int ThrowsCounted::throw_after = 0;

void test_ctor_throws() {
#ifndef TEST_HAS_NO_EXCEPTIONS
  using It = bidirectional_iterator<ThrowsCounted*>;
  const int N = 5;
  int values[N] = {1, 2, 3, 4, 5};
  alignas(ThrowsCounted) char pool[sizeof(ThrowsCounted)*N] = {};
  ThrowsCounted* p = (ThrowsCounted*)pool;
  try {
    ThrowsCounted::throw_after = 4;
    std::uninitialized_relocate_backward(values, values + N, It(p + N));
    assert(false);
  } catch (...) {}
  assert(ThrowsCounted::count == 0);  // the ones constructed have been destroyed automatically
  assert(ThrowsCounted::constructed == 4); // fourth construction throws
  assert(values[0] == 1);
  assert(values[1] == 2);
  assert(values[2] == 0);
  assert(values[3] == 0);
  assert(values[4] == 0);
#endif
}

void test_counted() {
  using It = bidirectional_iterator<int*>;
  using FIt = bidirectional_iterator<Counted*>;
  const int N = 5;
  int values[N] = {1, 2, 3, 4, 5};
  alignas(Counted) char pool[sizeof(Counted)*N] = {};
  Counted* p = (Counted*)pool;
  auto ret = std::uninitialized_relocate_backward(It(values), It(values + 1), FIt(p + 1));
  assert(ret == FIt(p));
  assert(Counted::constructed == 1);
  assert(Counted::count == 1);
  assert(p[0].value == 1);
  assert(values[0] == 0);
  ret = std::uninitialized_relocate_backward(It(values+1), It(values+N), FIt(p + N));
  assert(ret == FIt(p + 1));
  assert(Counted::count == 5);
  assert(Counted::constructed == 5);
  assert(p[1].value == 2);
  assert(p[2].value == 3);
  assert(p[3].value == 4);
  assert(p[4].value == 5);
  assert(values[1] == 0);
  assert(values[2] == 0);
  assert(values[3] == 0);
  assert(values[4] == 0);
  std::destroy(p, p + N);
  assert(Counted::count == 0);
}

void test_unique_ptr_rightward() {
  using T = std::unique_ptr<int>;
  alignas(T) char pool[sizeof(T) * 5];
  T *source = (T*)pool;
  for (int i=0; i < 3; ++i) {
    ::new ((void*)&source[i]) T(std::make_unique<int>(i));
  }
  auto result = std::uninitialized_relocate_backward(source, source + 3, source + 5);
  assert(result == source + 2);
  assert(*source[2] == 0);
  assert(*source[3] == 1);
  assert(*source[4] == 2);
  std::destroy(source + 2, source + 5);
}

void test_unique_ptr_leftward() {
  using T = std::unique_ptr<int>;
  alignas(T) char pool[sizeof(T) * 5];
  T *source = (T*)pool;
  for (int i=2; i < 5; ++i) {
    ::new ((void*)&source[i]) T(std::make_unique<int>(i));
  }
  auto result = std::uninitialized_relocate_backward(
    std::make_reverse_iterator(source + 5),
    std::make_reverse_iterator(source + 2),
    std::make_reverse_iterator(source)
  );
  assert(result == std::make_reverse_iterator(source + 3));
  assert(*source[0] == 2);
  assert(*source[1] == 3);
  assert(*source[2] == 4);
  std::destroy(source, source + 3);
}

void test_array_source()
{
    using T = int[2];
    struct U {
        bool constructed_ = false;
        explicit U(T&& t) { t[0] = 1; constructed_ = true; }
    };
    const int N = 5;
    alignas(T) char sourcepool[sizeof(T) * N];
    alignas(U) char destpool[sizeof(U) * N];
    T *source = (T*)sourcepool;
    U *dest = (U*)destpool;
    for (int i=0; i < N; ++i) {
        ::new ((void*)&source[i]) T;
    }
    source[2][0] = 2;
    source[3][0] = 3;
    source[4][0] = 4;
    auto result = std::uninitialized_relocate_backward(source, source + 3, dest + 3);
    assert(result == dest);
    assert(dest[0].constructed_ == true);
    assert(dest[1].constructed_ == true);
    assert(dest[2].constructed_ == true);
    assert(source[3][0] == 3);
    assert(source[4][0] == 4);
    std::destroy(dest, dest + 3);
    std::destroy(source + 3, source + N);
}

int main(int, char**)
{
  test_counted();
  test_ctor_throws();
  test_unique_ptr_rightward();
  test_unique_ptr_leftward();
  test_array_source();

  return 0;
}
