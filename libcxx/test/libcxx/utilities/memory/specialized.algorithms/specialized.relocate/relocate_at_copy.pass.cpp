...
struct Counted {
  ...
  Counted(const Counted& rhs) : m_data(rhs.m_data) { ++count; }
  ~Counted() { --count; }
  friend void operator&(Counted) = delete;
};
int Counted::count = 0;

struct __attribute__((trivially_relocatable)) RCounted {
  static int count;
  int m_data;
  RCounted(int i) : m_data(i) { ++count; }
  RCounted(const RCounted& rhs) : m_data(rhs.m_data) { ++count; }
  ~RCounted() { --count; }
