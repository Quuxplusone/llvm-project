...
static_assert(!is_tec<std::complex<float>>); // floating-point member
static_assert(!is_tec<std::complex<double>>); // floating-point member
static_assert(!is_tec<std::optional<char>>); // disengaged space isn't salient
...
static_assert(!is_tec<std::unique_ptr<T[], void(*)(T*)>>); // Deleter isn't salient
#if TEST_STD_VER >= 23
static_assert(!is_tec<std::expected<int, int>>); // padding
static_assert(!is_tec<std::expected<short, char>>); // padding
static_assert(!is_tec<std::unexpected<T>>); // T is non-tec
#endif
#if TEST_STD_VER >= 26
static_assert(!is_tec<std::optional<T&>>); // comparison is deep, not shallow
#endif

// Cases that seemingly can't be trivial
static_assert(!is_tec<std::array<int, 1>>);
static_assert(!is_tec<std::deque<T>::iterator>); // trouble with constexprness??
static_assert(!is_tec<std::deque<T>::const_iterator>); // trouble with constexprness??
static_assert(!is_tec<std::forward_list<T>::iterator>); // trouble with incomplete T
static_assert(!is_tec<std::forward_list<T>::const_iterator>); // trouble with incomplete T
...
static_assert(!is_tec<std::unique_ptr<T[]>>); // TODO
#if TEST_STD_VER >= 23
static_assert(!is_tec<std::expected<char, char>>); // TODO
#endif
...
void test_robust_against_adl(H *h, HH hh) {
  // Make sure each type can still be instantiated without error.
  {
    ...
    [[maybe_unused]] auto t15 = std::optional<H*>();
    [[maybe_unused]] auto t16 = std::pair<H*, H*>();
    [[maybe_unused]] auto t17 = std::pair<H*&, H*&>(h, h);
    [[maybe_unused]] auto t18 = std::set<H*>::iterator();
    [[maybe_unused]] auto t19 = std::set<H*>::const_iterator();
    [[maybe_unused]] auto t20 = std::tuple<H*>();
    [[maybe_unused]] auto t21 = std::tuple<H*&>(h);
    [[maybe_unused]] auto t22 = std::unique_ptr<H*>();
    [[maybe_unused]] auto t23 = std::vector<H*>::iterator();
    [[maybe_unused]] auto t24 = std::vector<H*>::const_iterator();
    [[maybe_unused]] auto t25 = std::views::elements<0>(HTupleView()).begin();
    [[maybe_unused]] auto t26 = std::views::iota(&h, &h).begin();
#if TEST_STD_VER >= 23
    [[maybe_unused]] auto t27 = std::expected<H*,H*>();
    [[maybe_unused]] auto t28 = std::unexpected<H*>(nullptr);
    [[maybe_unused]] auto t29 = std::views::zip(HView(), HView()).begin();
#endif
#if TEST_STD_VER >= 26
    [[maybe_unused]] auto t30 = std::optional<H*&>();
#endif
  }
  {
    ...
    [[maybe_unused]] auto t15 = std::optional<HH>();
    [[maybe_unused]] auto t16 = std::pair<HH, HH>();
    [[maybe_unused]] auto t17 = std::pair<HH&, HH&>(hh, hh);
    [[maybe_unused]] auto t18 = std::set<HH>::iterator();
    [[maybe_unused]] auto t19 = std::set<HH>::const_iterator();
    [[maybe_unused]] auto t20 = std::tuple<HH>();
    [[maybe_unused]] auto t21 = std::tuple<HH&>(hh);
    [[maybe_unused]] auto t22 = std::unique_ptr<HH>();
    [[maybe_unused]] auto t23 = std::vector<HH>::iterator();
    [[maybe_unused]] auto t24 = std::vector<HH>::const_iterator();
    [[maybe_unused]] auto t25 = std::views::elements<0>(HHTupleView()).begin();
    [[maybe_unused]] auto t26 = std::views::iota(std::addressof(hh), std::addressof(hh)).begin();
#if TEST_STD_VER >= 23
    [[maybe_unused]] auto t27 = std::expected<HH, HH>();
    [[maybe_unused]] auto t28 = std::unexpected<HH>(hh);
    [[maybe_unused]] auto t29 = std::views::zip(HHView(), HHView()).begin();
#endif
#if TEST_STD_VER >= 26
    [[maybe_unused]] auto t30 = std::optional<HH&>();
#endif
  }
}
