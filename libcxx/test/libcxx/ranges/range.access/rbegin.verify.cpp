...
void baditerator() {
  ...
  // expected-note@*:* {{because 'derived_from<_ITER_CONCEPT<decltype(static_cast<typename decay<decltype((__t.begin()))>::type>(__t.begin()))>, bidirectional_iterator_tag>' evaluated to false}}
  // expected-note@*:* {{because 'is_base_of_v<bidirectional_iterator_tag, _ITER_CONCEPT<decltype(static_cast<typename decay<decltype((__t.begin()))>::type>(__t.begin()))> >' evaluated to false}}
