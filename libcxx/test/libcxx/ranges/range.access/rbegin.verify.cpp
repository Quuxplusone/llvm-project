...
void baditerator() {
  ...
  // expected-note@*:* {{because 'derived_from<_ITER_CONCEPT<std::__forward_list_iterator<std::__forward_list_node<int, void *> *> >, std::bidirectional_iterator_tag>' evaluated to false}}
...
void rvalue() {
  ...
  // expected-note@*:* {{and 'enable_borrowed_range<remove_cvref_t<std::string> >' evaluated to false}}
  // expected-note@*:* {{candidate template ignored: constraints not satisfied}}
