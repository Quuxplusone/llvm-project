...
int main(int, char**)
{
    ...
    // Try catching some exotic types
    {
        ...
        // The commented-out lines suffer from the problem
        // described here, such that the returned pointer dangles:
        // https://quuxplusone.github.io/blog/2023/11/17/reinterpret-cast-via-throw/

        int i = 42;
        p = std::make_exception_ptr(&i);
        //assert(std::try_cast<int*>(p) != nullptr);
        //assert(*std::try_cast<int*>(p) == &i);
        //assert(std::try_cast<void*>(p) != nullptr);
        //assert(*std::try_cast<void*>(p) == &i);
        //assert(std::try_cast<const int*>(p) != nullptr);
        //assert(*std::try_cast<const int*>(p) == &i);
        //assert(std::try_cast<const void*>(p) != nullptr);
        //assert(*std::try_cast<const void*>(p) == &i);

        p = std::make_exception_ptr((const int*)&i);
        assert(std::try_cast<int*>(p) == nullptr);
        assert(std::try_cast<void*>(p) == nullptr);
        //assert(std::try_cast<const int*>(p) != nullptr);
        //assert(*std::try_cast<const int*>(p) == &i);
        ...
        Concrete c;
        p = std::make_exception_ptr(&c);
        //assert(std::try_cast<Abstract*>(p) != nullptr);
        //assert(*std::try_cast<Abstract*>(p) == &c);

        // Catching Incomplete or Incomplete* is forbidden,
        // but catching Incomplete** is allowed.
        Incomplete *ip;
        p = std::make_exception_ptr(&ip);
        //assert(std::try_cast<Incomplete**>(p) != nullptr);
        //assert(*std::try_cast<Incomplete**>(p) == &ip);
        assert(std::try_cast<const Incomplete**>(p) == nullptr);
        //assert(std::try_cast<Incomplete* const*>(p) != nullptr);
        //assert(*std::try_cast<Incomplete* const*>(p) == &ip);
    }
    return 0;
}
