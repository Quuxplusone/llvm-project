...
TEST_CONSTEXPR_CXX20 bool tests() {
  {
    ...
    std::vector vec(arr, arr + 10);
    ASSERT_SAME_TYPE(decltype(vec), std::vector<int>);
    assert(std::equal(vec.begin(), vec.end(), arr, arr + 10));
  }
  {
    const long arr[] = { INT_MAX, 1L, 2L, 3L };
    std::vector vec(arr, arr + 4, std::allocator<long>());
  ...
  {
    int a[2] = {1,2};
    std::vector vec(std::from_range, a);
    ASSERT_SAME_TYPE(decltype(vec), std::vector<int>);
    assert(vec.size() == 2);
  }
  {
    int a[2] = {1,2};
    std::vector vec(std::from_range, a, test_allocator<int>());
    ASSERT_SAME_TYPE(decltype(vec), std::vector<int, test_allocator<int>>);
    assert(vec.size() == 2);
  }
#endif
  ...
  {
    bool a[2] = {true,true};
    std::vector vec(std::from_range, a, test_allocator<bool>());
    ASSERT_SAME_TYPE(decltype(vec), std::vector<bool, test_allocator<bool>>);
    assert(vec.size() == 2);
  }
#endif
  {
