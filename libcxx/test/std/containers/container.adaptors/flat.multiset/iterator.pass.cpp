...
{
  {
    ...
    assert(m.size() == 24);
    assert(std::distance(m.begin(), m.end()) == 24);
    assert(std::distance(m.cbegin(), m.cend()) == 24);
    assert(std::distance(m.rbegin(), m.rend()) == 24);
    assert(std::distance(m.crbegin(), m.crend()) == 24);
    ...
    for (int j = 1; j <= 8; ++j) {
      ...
      ++i;
      assert(*i++ == j);  // post-increment, dereference
      assert(*i++ == j);  // post-increment, dereference
    }
    assert(i == m.end());
    for (int j = 8; j >= 1; --j) {
  ...
  {
    using M = std::flat_multiset<short, std::less<>, std::deque<short>>;
