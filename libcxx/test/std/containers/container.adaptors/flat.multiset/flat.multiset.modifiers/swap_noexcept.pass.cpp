...
int main(int, char**)
{
  ...
  {
    using C = std::flat_multiset<MoveOnly, std::less<MoveOnly>, std::deque<MoveOnly, test_allocator<MoveOnly>>>;
    C c;
    static_assert(noexcept(c.swap(c)));
    static_assert(noexcept(swap(c, c)));
    static_assert(std::is_nothrow_swappable_v<C>);
  ...
  {
    using C = std::flat_multiset<MoveOnly, std::greater<>, std::vector<MoveOnly, other_allocator<MoveOnly>>>;
    C c;
    static_assert(noexcept(c.swap(c)));
    static_assert(noexcept(swap(c, c)));
    static_assert(std::is_nothrow_swappable_v<C>);
  }
  {
    ...
    using C = std::flat_multiset<MoveOnly, ThrowingSwapComp>;
    C c;
    static_assert(noexcept(c.swap(c)));
    static_assert(noexcept(swap(c, c)));
    static_assert(std::is_nothrow_swappable_v<C>);
