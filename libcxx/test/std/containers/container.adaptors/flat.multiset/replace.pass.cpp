...
{
  {
    ...
    M m = {1, 5, 4, 9, 2};
    V v = {2, 2, 3, 5}; // "Preconditions: The elements of cont are sorted with respect to compare."
  ...
  }
  {
    ...
    using M = std::flat_multiset<int, std::function<bool(int,int)>, V>;
    M m({1, 3, 6, 10}, std::greater<int>());
    V v = {4, 4, 3}; // "Preconditions: The elements of cont are sorted with respect to compare."
    ASSERT_SAME_TYPE(decltype(m.replace(std::move(v))), void);
    m.replace(std::move(v));
    assert(v.empty());  // because it's been moved-from
    assert(m.key_comp()(2, 1) == true); // the comparator hasn't been changed
    assert(m == M({4, 4, 3}, std::greater<int>()));
  }
  {
