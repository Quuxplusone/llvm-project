...
int main(int, char**)
{
  ...
  {
    using M = std::flat_multiset<int, std::less<int>, std::deque<int, min_allocator<int>>>;
    std::deque<int, min_allocator<int>> container = {3,4};
    container.insert(container.begin(), {1,2}); // now container is [_ 1 2] [3 4 _]
    M m = M(std::sorted_equivalent, std::move(container));
    ASSERT_SAME_TYPE(decltype(m.erase(m.begin())), M::iterator);
    ASSERT_SAME_TYPE(decltype(m.erase(m.cbegin())), M::iterator);
  ...
  {
    test_lwg2059< std::flat_multiset<KeyConvertibleFromIterator> >();
    test_lwg2059< std::flat_multiset<KeyConvertibleFromIterator, std::less<>> >();
  }
  {
    test_p2077< std::flat_multiset<int, std::less<>, std::vector<int>> >();
    test_p2077< std::flat_multiset<int, std::less<>, std::deque<int>> >();
    test_p2077< std::flat_multiset<bool, std::less<>, std::vector<bool>> >();
    test_p2077< std::flat_multiset<bool, std::less<>, std::deque<bool>> >();
  }
  return 0;
}
