...
template<class T>
concept HasExtract = requires (T&& t) {
  { std::forward<T>(t).extract() };
};
...
{
  {
    ...
    assert((v == std::vector<int>{1, 2, 4, 5, 5, 9}));
  }
  {
    using M = std::flat_multiset<int, MoveSensitiveComp, std::deque<int, min_allocator<int>>>;
    M m = {1, 1, 3, 6, 10};
    static_assert(!HasExtract<M&>);
    static_assert(!HasExtract<const M&>);
    static_assert(HasExtract<M&&>);
    ...
    assert((v == std::deque<int, min_allocator<int>>{1, 1, 3, 6, 10}));
  }
  {
    // Extracting moves (not copies) the container
    using M = std::flat_multiset<Counter<int>>;
    M m = {1, 2, 3, 4, 5, 6};
  ...
  }
  {
    ...
    std::flat_multiset<int, std::less<>, CopyOnlyContainer> m;
    ...
    assert(m.empty());            // ...the flat_multiset is emptied anyway
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  {
    // *this is emptied, even if the function exits via an exception.
    std::flat_multiset<int, std::less<>, ThrowingMoveContainer> m = {1, 2, 3, 4};
    try {
      std::move(m).extract();
      assert(false);
    } catch (int ex) {
