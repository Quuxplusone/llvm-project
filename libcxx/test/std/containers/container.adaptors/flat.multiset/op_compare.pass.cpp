...
int main(int, char**) {
  {
    ...
    s2 = {1,2,4};  // {1} versus {1,2,4}
    assert(testComparisons(s1, s2, false, true));
    s1 = {1,2,3,4};  // {1,2,3,4} versus {1,2,4}
    assert(testComparisons(s1, s2, false, true));
    s2 = {1,3,4};  // {1,2,3,4} versus {1,3,4}
    assert(testComparisons(s1, s2, false, true));
  }
  {
    // Comparisons use value_type's native operators, not the comparator
    using C = std::flat_multiset<int, std::greater<int>>;
    C s1 = {1};
    C s2 = {2};  // {1} versus {2}
    ASSERT_SAME_TYPE(decltype(s1 <=> s2), std::strong_ordering);
    AssertComparisonsReturnBool<C>();
    ...
    s2 = {1};    // {1} versus {1}
    assert(testComparisons(s1, s2, true, false));
    s2 = {1,2,4};  // {1} versus {4,2,1}
    assert(testComparisons(s1, s2, false, true));
    s1 = {1,2,3,4};  // {4,3,2,1} versus {4,2,1}
    assert(testComparisons(s1, s2, false, false));
    s2 = {1,3,4};  // {4,3,2,1} versus {4,3,1}
    assert(testComparisons(s1, s2, false, false));
  }
  {
    using C = std::flat_multiset<double>;
    C s1 = {1};
    C s2 = { std::numeric_limits<double>::quiet_NaN() };
  ...
  }
  {
    ...
    AssertComparisonsReturnBool<C>();
    assert(testComparisonsComplete(s1, s2, false, false, false));
    s1 = { 1, std::numeric_limits<double>::quiet_NaN() };
    s2 = { std::numeric_limits<double>::quiet_NaN(), 1 };
    assert(std::lexicographical_compare_three_way(s1.begin(), s1.end(), s2.begin(), s2.end(), std::strong_order) == std::strong_ordering::equal);
    assert(s1 != s2);
