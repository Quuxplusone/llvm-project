...
{
  {
    ...
    int ar[] = {1,2,2,5};
    auto m = M(std::sorted_equivalent, cpp17_input_iterator<const int*>(ar), cpp17_input_iterator<const int*>(ar + 4), std::less<int>());
    assert(m == M({1,2,2,5}, std::less<>()));
    assert(m.key_comp()(1, 2) == true);
  }
  {
    using M = std::flat_multiset<int, std::greater<int>, std::deque<int, min_allocator<int>>>;
    int ar[] = {5,4,2,2};
    auto m = M(std::sorted_equivalent, cpp17_input_iterator<const int*>(ar), cpp17_input_iterator<const int*>(ar + 4), std::greater<int>());
    assert((m == M{5,4,2,2}));
  }
  {
  ...
  }
  {
    ...
    int ar[] = {1,2,5,5};
    auto m = M(std::sorted_equivalent, ar, ar + 4, C(3), A(5));
    assert((m == M{1,2,5,5}));
  ...
  }
  {
    ...
    int ar[] = {1,2,5,5};
    M m = { std::sorted_equivalent, ar, ar + 4, {}, A(5) }; // implicit ctor
    assert((m == M{1,2,5,5}));
    assert(std::move(m).extract().get_allocator() == A(5));
  }
  {
    ...
    int ar[] = {1,2,5,5};
    vm.emplace_back(std::sorted_equivalent, cpp17_input_iterator<const int*>(ar), cpp17_input_iterator<const int*>(ar + 4), C(3));
    assert((vm[0] == M{1,2,5,5}));
