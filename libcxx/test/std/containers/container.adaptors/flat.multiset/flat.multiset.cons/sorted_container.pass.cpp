...
// template<class Allocator>
//   flat_multiset(sorted_equivalent_t, container_type&& cont, const Allocator& a);
...
{
  {
    ...
    std::vector<int> v = {1,4,4,10};
    auto m = M(std::sorted_equivalent, v);
    assert((m == M{1,4,4,10}));
    m = M(std::sorted_equivalent, std::move(v));
  ...
  }
  {
    ...
    auto v = std::vector<int, A>({1,2,4,4}, A(5));
    auto m = M(std::sorted_equivalent, std::move(v));
    assert(v.empty()); // it was moved-from
    assert((m == M{1,2,4,4}));
  ...
  }
  {
    using A = test_allocator<int>;
    using M = std::flat_multiset<int, std::less<int>, std::vector<int, A>>;
    auto v = std::vector<int, A>({1,2,4,4}, A(4));
    auto m = M(std::sorted_equivalent, v, A(5)); // replaces v's allocator
  ...
  }
  {
    ...
    auto v = std::deque<int, A>({1,2,4,4}, A(4));
    M m = { std::sorted_equivalent, v, A(5) }; // implicit ctor
    assert(!v.empty()); // it was an lvalue above
    assert((m == M{1,2,4,4}));
    assert(std::move(m).extract().get_allocator() == A(5));
  }
  ...
  }
  {
    ...
    std::pmr::monotonic_buffer_resource mr;
    std::pmr::vector<M> vm(&mr);
    std::pmr::vector<int> v = {1,4,4,10};
    assert(v.get_allocator().resource() != &mr);
    vm.emplace_back(std::sorted_equivalent, std::move(v));
    LIBCPP_ASSERT(v.size() == 4); // v is untouched, since it uses a different allocator
    assert((vm[0] == M{1,4,4,10}));
    assert(std::move(vm[0]).extract().get_allocator().resource() == &mr);
  }
  {
    using M = std::flat_multiset<int, std::less<int>, std::pmr::vector<int>>;
    std::pmr::monotonic_buffer_resource mr;
    std::pmr::vector<M> vm(&mr);
    std::pmr::vector<int> v({1,4,4,10}, &mr);
    assert(v.get_allocator().resource() == &mr);
    vm.emplace_back(std::sorted_equivalent, std::move(v));
  ...
  }
  {
    ...
    vm.emplace_back(std::sorted_equivalent, std::move(v)); // this was a hard error before LWG 3802
    assert(vm.size() == 1);
  }
  return 0;
}
