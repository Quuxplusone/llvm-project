...
{
  {
    ...
    M m = {{1,'a'}, {1,'b'}, {2,'b'}, {4,'d'}, {4,'d'}, {5,'e'}, {8,'h'}, {8,'g'}, {8,'h'}};
    ...
    assert(m.upper_bound(2) == m.begin() + 3);
    assert(m.upper_bound(3) == m.begin() + 3);
    assert(m.upper_bound(4) == m.begin() + 5);
    assert(m.upper_bound(5) == m.begin() + 6);
    assert(m.upper_bound(6) == m.begin() + 6);
    assert(std::as_const(m).upper_bound(7) == m.begin() + 6);
    assert(std::as_const(m).upper_bound(8) == m.end());
    assert(std::as_const(m).upper_bound(9) == m.end());
  }
  {
    using M = std::flat_multimap<int, char, std::greater<int>, std::deque<int, min_allocator<int>>, std::deque<char, min_allocator<char>>>;
    M m = {{1,'a'}, {1,'b'}, {2,'b'}, {4,'d'}, {4,'d'}, {5,'e'}, {8,'h'}, {8,'g'}, {8,'h'}};
    ASSERT_SAME_TYPE(decltype(m.upper_bound(0)), M::iterator);
    ASSERT_SAME_TYPE(decltype(std::as_const(m).upper_bound(0)), M::const_iterator);
    assert(m.upper_bound(0) == m.end());
    assert(m.upper_bound(1) == m.end());
    assert(m.upper_bound(2) == m.begin() + 7);
    assert(m.upper_bound(3) == m.begin() + 6);
