...
{
  {
    ...
    M m = {{1,'a'}, {2,'b'}, {4,'d'}, {5,'e'}, {5,'f'}, {5,'g'}, {8,'h'}};
    ...
    assert(m.find(5) != m.end() && m.find(5)->first == 5);
  ...
  {
    using M = std::flat_multimap<int, char, std::greater<int>, std::deque<int, min_allocator<int>>, std::string>;
    M m = {{1,'a'}, {2,'b'}, {4,'d'}, {5,'e'}, {5,'f'}, {5,'g'}, {8,'h'}};
    ASSERT_SAME_TYPE(decltype(m.find(0)), M::iterator);
    ASSERT_SAME_TYPE(decltype(std::as_const(m).find(0)), M::const_iterator);
    assert(m.find(0) == m.end());
