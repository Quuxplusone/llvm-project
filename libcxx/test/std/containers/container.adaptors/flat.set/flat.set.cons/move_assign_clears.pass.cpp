...
{
  {
    ...
    using M = std::flat_set<MoveNegates, std::less<MoveNegates>, NaiveStaticVector<MoveNegates, 10>>;
  ...
  {
    const int expected[] = {1,2,3,4,5,6,7,8};
    using M = std::flat_set<MoveClears, std::less<MoveClears>, NaiveStaticVector<MoveClears, 10>>;
    M m = M(expected, expected + 8);
    M m2 = M(expected, expected + 3);
