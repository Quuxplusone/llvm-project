...
void test_container() {
  ...
  }
  {
    ...
    ASSERT_SAME_TYPE(decltype(s), std::flat_set<int, std::less<int>, std::pmr::vector<int>>);
    assert(std::ranges::equal(s, expected));
  ...
  }
  {
    std::pmr::monotonic_buffer_resource mr;
    std::pmr::monotonic_buffer_resource mr2;
    std::pmr::vector<int> pv({ 1, 2, 3, INT_MAX }, &mr);
...
void test_container_compare() {
  ...
  }
  {
    ...
    ASSERT_SAME_TYPE(decltype(s), std::flat_set<int, std::greater<int>, std::pmr::vector<int>>);
    assert(std::ranges::equal(s, expected));
    assert(std::move(s).extract().get_allocator().resource() == &mr2);
  }
  {
    std::pmr::monotonic_buffer_resource mr;
    std::pmr::monotonic_buffer_resource mr2;
    std::pmr::vector<int> pv({ INT_MAX, 3, 2, 1 }, &mr);
