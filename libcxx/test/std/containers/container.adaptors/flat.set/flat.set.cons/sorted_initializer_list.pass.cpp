...
// template<class Alloc> flat_set(sorted_unique_t, initializer_list<value_type> il, const Alloc& a);
...
int main(int, char**)
{
  int expected[] = {1,2,3,5};
  {
    ...
    M m = M(std::sorted_unique, {1,2,3,5});
    assert(m.size() == 4);
  ...
  }
  {
    ...
    M m = M(std::sorted_unique, {5,3,2,1});
    assert(m.size() == 4);
    assert(std::equal(m.rbegin(), m.rend(), expected, expected+4));
  }
  {
    using M = std::flat_set<int, std::greater<int>, std::deque<int, min_allocator<int>>>;
    std::initializer_list<int> il = {5,3,2,1};
    M m = M(std::sorted_unique, il);
    assert(m.size() == 4);
    assert(std::equal(m.rbegin(), m.rend(), expected, expected+4));
  }
  {
    using M = std::flat_set<short>;
    M m = M(std::sorted_unique, {1,2,3,5}); // ints implicitly converted to shorts
    assert(m.size() == 4);
    assert(std::equal(m.begin(), m.end(), expected, expected+4));
  ...
  }
  {
    using A = explicit_allocator<int>;
    {
      ...
      M m = M(std::sorted_unique, {42});
      assert(m == M{42});
      assert(m.key_comp().default_constructed_);
    }
    {
      A a;
      std::flat_set<int, std::greater<int>, std::deque<int, A>> m(std::sorted_unique, {5,3,2,1}, a);
      assert(std::equal(m.rbegin(), m.rend(), expected, expected+4));
    }
  }
  {
    ...
    vm.emplace_back(std::sorted_unique, il);
