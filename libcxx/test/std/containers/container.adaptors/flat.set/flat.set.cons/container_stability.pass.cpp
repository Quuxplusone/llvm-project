...
{
  {
    ...
    std::flat_set<uint16_t, Mod256> fs(std::vector<uint16_t>(values, values + 200));
  ...
  }
  {
    ...
    std::set<uint16_t, Mod256> s(values, values + 200);
    std::flat_set<uint16_t, Mod256> fs(std::vector<uint16_t>(values, values + 200), std::allocator<int>());
    assert(fs.size() == s.size());
    LIBCPP_ASSERT(std::ranges::equal(fs, s));
  }
  return 0;
