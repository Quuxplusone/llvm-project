...
int main(int, char**)
{
...
#endif // _LIBCPP_VERSION
  {
    ...
    using C = std::flat_set<int, ThrowingCopyComp>;
    static_assert(!std::is_nothrow_move_constructible_v<C>);
    C c;
    C d = std::move(c);
  }
  {
    // Moving the flat_set copies the comparator (to support std::function comparators)
    using C = std::flat_set<int, MoveSensitiveComp>;
    LIBCPP_STATIC_ASSERT(std::is_nothrow_move_constructible_v<C>);
    C c;
