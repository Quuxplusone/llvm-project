...
{
  {
    using A = limited_allocator<int, 10>;
    using C = std::flat_set<int, std::less<int>, std::vector<int, A>>;
    ASSERT_SAME_TYPE(C::difference_type, std::vector<int, A>::difference_type);
    ASSERT_SAME_TYPE(C::size_type, std::vector<int, A>::size_type);
    C c;
