...
int main(int, char**)
{
  ...
  {
    // insert(K&&)
    using M = std::flat_set<CompareCounter, std::less<>>;
    M m = {1,2,4,5};
    expensive_comparisons = 0;
    cheap_comparisons = 0;
    std::same_as<std::pair<M::iterator, bool>> auto p = m.insert(3); // conversion happens last
  ...
  {
    // insert(value_type&&)
    using M = std::flat_set<CompareCounter>;
    M m = {1,2,4,5};
    expensive_comparisons = 0;
    cheap_comparisons = 0;
    std::same_as<std::pair<M::iterator, bool>> auto p = m.insert(3); // conversion happens last
  ...
  }
  {
    ...
    expensive_comparisons = 0;
    cheap_comparisons = 0;
    std::same_as<M::iterator> auto p = m.insert(m.begin(), 3);
    assert(expensive_comparisons >= 2);
    assert(cheap_comparisons == 0);
    assert(p == m.begin() + 2);
    assert(std::ranges::equal(m, expected));
  }
  {
    ...
    expensive_comparisons = 0;
    cheap_comparisons = 0;
    std::same_as<std::pair<M::iterator, bool>> auto p = m.emplace(3); // conversion happens first
    assert(expensive_comparisons >= 2);
    assert(cheap_comparisons == 0);
    assert(p == std::make_pair(m.begin() + 2, true));
    assert(std::ranges::equal(m, expected));
  }
  {
    ...
    using M = std::flat_set<Address, std::less<>>;
    int a, b, c, d, e;
    M m = {&a, &b};
    M m2 = {&d, &e};
    assert(m.size() == 2);
    m.insert(m2.begin()); // insert(K&&)
    ASSERT_SAME_TYPE(decltype(m.insert(m2.begin())), std::pair<M::iterator, bool>);
    assert(m.size() == 3);
