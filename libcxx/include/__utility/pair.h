...
template <class _T1, class _T2>
struct _LIBCPP_TRIVIALLY_RELOCATABLE_IF((__libcpp_is_trivially_relocatable<_T1>::value && __libcpp_is_trivially_relocatable<_T2>::value)) pair
...
#endif
{
  ...
  pair& operator=(pair&&) __attribute__((__enable_if__(__has_defaulted_members, ""))) = default;
#else
...
#  endif
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 pair& operator=(
      __conditional_t<!__has_defaulted_members && is_move_assignable<first_type>::value && is_move_assignable<second_type>::value, pair, __nat>&&
