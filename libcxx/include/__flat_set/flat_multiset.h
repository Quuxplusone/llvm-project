...
class _LIBCPP_TRIVIALLY_RELOCATABLE_IF(__libcpp_is_trivially_relocatable<_KeyContainer>::value && __libcpp_is_trivially_relocatable<_Compare>::value) flat_multiset {
...
public:
  ...
  template <class _InputIterator, class _Alloc>
    requires __is_cpp17_input_iterator<_InputIterator>::value && uses_allocator_v<container_type, _Alloc>
  _LIBCPP_HIDE_FROM_ABI
  ...
  _LIBCPP_HIDE_FROM_ABI
  flat_multiset(from_range_t __fr, _Rp&& __r)
    : flat_multiset(__fr, std::forward<_Rp>(__r), key_compare()) {}
  ...
  template <__container_compatible_range<value_type> _Rp>
  _LIBCPP_HIDE_FROM_ABI
  flat_multiset(from_range_t, _Rp&& __r, const key_compare& __comp)
  ...
  _LIBCPP_HIDE_FROM_ABI
  void insert(sorted_equivalent_t __s, std::initializer_list<value_type> __il) { insert(__s, __il.begin(), __il.end()); }
...
private:
  ...
  _LIBCPP_HIDE_FROM_ABI
  bool __is_sorted(iterator __first, iterator __last) const {
