...
_LIBCPP_HIDE_FROM_ABI const __remove_cvref_t<_Ep>* try_cast(const exception_ptr& __p) _NOEXCEPT {
  ...
  try {
    ...
    try {
      ...
      _LIBCPP_ASSERT_INTERNAL(std::addressof(__ex) == std::addressof(__also_ex), "this should never happen");
    }
...
#else
  ((void)__p);
  _LIBCPP_VERBOSE_ABORT("try_cast was called in -fno-exceptions mode");
#endif // _LIBCPP_HAS_EXCEPTIONS
}
