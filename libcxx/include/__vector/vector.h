...
class _LIBCPP_TRIVIALLY_RELOCATABLE_IF((__vector_be_trivially_relocatable<_Allocator>::value))
      _LIBCPP_WARN_UNUSED vector {
...
public:
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI value_type displace(const_iterator __pos) {
    ...
    if (!__libcpp_is_constant_evaluated()) {
      pointer __p = this->__layout_.__begin_ptr() + (__pos - begin());
      value_type __v = std::relocate(std::addressof(*__p));
      pointer __endp = this->__layout_.__end_ptr();
      std::uninitialized_relocate(__p + 1, __endp, __p);
      this->__layout_.__set_bound_using_pointer(__endp - 1);
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI value_type displace_back() {
    if (!__libcpp_is_constant_evaluated()) {
      ...
      this->__layout_.__set_bound_using_pointer(this->__layout_.__end_ptr() - 1);
      __annotate_shrink(size() + 1);
      return __v;
    } else {
...
private:
  ...
  template <class _Up>
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI void
  __relocate_upward_and_insert_single(pointer __from_s, pointer __from_e, _Up&& __elt);
  template <class _AlgPolicy, class _Iterator>
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI void
  __relocate_upward_and_insert_range(pointer __from_s, pointer __from_e, size_type __upward, _Iterator __first);
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI void __destruct_at_end(pointer __new_last, false_type = false_type()) _NOEXCEPT {
    size_type __old_size = size();
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI void __destruct_at_end(pointer __new_last, true_type) _NOEXCEPT {
    size_type __old_size = size();
    this->__layout_.__set_bound_using_pointer(__new_last);
    __annotate_shrink(__old_size);
  }
...
vector<_Tp, _Allocator>::erase(const_iterator __position) {
  ...
  if (__vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value && !__libcpp_is_constant_evaluated()) {
    __alloc_traits::destroy(this->__layout_.__alloc(), std::__to_address(__p));
    ::__builtin_memmove((void*)std::__to_address(__p), (void*)std::__to_address(__p + 1), (this->__layout_.__end_ptr() - (__p + 1)) * sizeof(_Tp));
    this->__destruct_at_end(this->__layout_.__end_ptr() - 1, true_type()); // destroy_via_noop
  } else {
    this->__destruct_at_end(std::move(__p + 1, this->__layout_.__end_ptr(), __p));
  }
...
vector<_Tp, _Allocator>::erase(const_iterator __first, const_iterator __last) {
  ...
  if (__first != __last) {
    if (__vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value && !__libcpp_is_constant_evaluated()) {
      pointer __endp = __p + (__last - __first);
      for (pointer __q = __p; __q != __endp; ++__q) {
        __alloc_traits::destroy(this->__layout_.__alloc(), std::__to_address(__q));
      }
      ::__builtin_memmove((void*)std::__to_address(__p), (void*)std::__to_address(__endp), (this->__layout_.__end_ptr() - __endp) * sizeof(_Tp));
      this->__destruct_at_end(this->__layout_.__end_ptr() - (__last - __first), true_type()); // destroy_via_noop
    } else {
      this->__destruct_at_end(std::move(__p + (__last - __first), this->__layout_.__end_ptr(), __p));
    }
  }
  return __make_iter(__p);
}
...
vector<_Tp, _Allocator>::__relocate_upward_and_insert_single(pointer __from_s, pointer __from_e, _Up&& __elt)
{
  if (!__libcpp_is_constant_evaluated() && __vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value) {
    ...
    try {
#endif // _LIBCPP_HAS_EXCEPTIONS
      __alloc_traits::construct(this->__layout_.__alloc(),
                                std::__to_address(__from_s),
                                static_cast<_Up&&>(__elt));
#if _LIBCPP_HAS_EXCEPTIONS
    } catch (...) {
...
template<class _AlgPolicy, class _Iterator>
_LIBCPP_CONSTEXPR_SINCE_CXX20 void
vector<_Tp, _Allocator>::__relocate_upward_and_insert_range(pointer __from_s, pointer __from_e, size_type __upward,
                                                            _Iterator __first)
{
  if (!__libcpp_is_constant_evaluated() && __vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value) {
    size_type __from_n = __from_e - __from_s;
    // When __upward exceeds __from_n, the relocated elements leapfrog over
    // already-constructed elements and the bottom of the window (that we'll
    // lower back down in case of exception) becomes __from_n, not __upward.
    ...
    try {
      ...
      while (__i < __upward) {
        __alloc_traits::construct(this->__layout_.__alloc(),
                                  std::__to_address(__from_s) + __i,
                                  *__first);
        ++__i; // the element is now constructed
        this->__layout_.__set_bound_using_pointer(this->__layout_.__end_ptr() + 1);
        ++__first;
      }
#if _LIBCPP_HAS_EXCEPTIONS
      ...
      ::__builtin_memmove((void*)(__window_sill + __i), (void*)(__window_sill + __upward), __from_n * sizeof(_Tp));
      throw;
    }
#endif // _LIBCPP_HAS_EXCEPTIONS
  } else {
    __move_range(__from_s, __from_e, __from_s + __upward);
    __insert_assign_n_unchecked<_AlgPolicy>(std::move(__first), __upward, __from_s);
  }
}
  ...
  if (size() != capacity()) {
    ...
    if (__p == __end) {
      ...
      if (std::__is_pointer_in_range(std::__to_address(__p), std::__to_address(__end), std::addressof(__x)))
        ++__xr;
      __relocate_upward_and_insert_single(__p, this->__layout_.__end_ptr(), *__xr);
    }
  } else {
...
vector<_Tp, _Allocator>::insert(const_iterator __position, value_type&& __x) {
  ...
  if (size() != capacity()) {
    ...
    if (__p == __end) {
      ...
      __relocate_upward_and_insert_single(__p, this->__layout_.__end_ptr(), std::move(__x));
    }
  } else {
    _SplitBuffer __v(__recommend(size() + 1), __p - this->__layout_.__begin_ptr(), this->__layout_.__alloc());
...
vector<_Tp, _Allocator>::emplace(const_iterator __position, _Args&&... __args) {
  ...
  if (size() != capacity()) {
    ...
    if (__p == __end) {
      ...
      __relocate_upward_and_insert_single(__p, this->__layout_.__end_ptr(), std::move(__tmp.get()));
    }
  } else {
    _SplitBuffer __v(__recommend(size() + 1), __p - this->__layout_.__begin_ptr(), this->__layout_.__alloc());
    __v.emplace_back(std::forward<_Args>(__args)...);
...
vector<_Tp, _Allocator>::__insert_with_size(
  ...
  if (__n > 0) {
    if (__n <= static_cast<difference_type>(__layout_.__remaining_capacity())) {
      ...
      if (!__libcpp_is_constant_evaluated() && __vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value) {
        __relocate_upward_and_insert_range<_AlgPolicy>(__p, __old_last, __n, std::move(__first));
      } else if (__n > __dx) {
#if _LIBCPP_STD_VER >= 23
