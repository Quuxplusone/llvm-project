...
class _LIBCPP_TEMPLATE_VIS
    _LIBCPP_TRIVIALLY_RELOCATABLE_IF((__vector_be_trivially_relocatable<_Allocator>::value))
    vector {
private:
...
private:
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI void
  __relocate_upward_and_insert_range(pointer __from_s, pointer __from_e, size_type __upward, _ForwardIterator __first, _ForwardIterator __last);
...
vector<_Tp, _Allocator>::erase(const_iterator __position) {
  ...
  if (__vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value && !__libcpp_is_constant_evaluated()) {
    ...
    ::__builtin_memmove(std::__to_address(__p), std::__to_address(__p + 1), (this->__end_ - (__p + 1)) * sizeof(_Tp));
  ...
  if (__first != __last) {
    if (__vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value && !__libcpp_is_constant_evaluated()) {
      ...
      for (pointer __q = __p; __q != __endp; ++__q) {
        __alloc_traits::destroy(__alloc(), std::__to_address(__q));
      }
      ::__builtin_memmove(std::__to_address(__p), std::__to_address(__endp), (this->__end_ - __endp) * sizeof(_Tp));
                                                            ...
                                                            _ForwardIterator __first, _ForwardIterator __last)
{
  if (!__libcpp_is_constant_evaluated() && __vector_relocate_elements_via_memcpy<_Tp, _Allocator>::value) {
    ...
    std::copy(__first, __last, __from_s);
  }
}
  ...
  if (this->__end_ < this->__end_cap()) {
    if (__p == this->__end_) {
      ...
      if (std::__is_pointer_in_range(std::__to_address(__p), std::__to_address(__end_), std::addressof(__x)))
        ++__xr;
      __relocate_upward_and_insert_single(__p, this->__end_, *__xr);
    }
  } else {
    allocator_type& __a = this->__alloc();
...
vector<_Tp, _Allocator>::insert(const_iterator __position, value_type&& __x) {
  ...
  if (this->__end_ < this->__end_cap()) {
    if (__p == this->__end_) {
      ...
      __relocate_upward_and_insert_single(__p, this->__end_, std::move(__x));
    }
  } else {
    allocator_type& __a = this->__alloc();
    __split_buffer<value_type, allocator_type&> __v(__recommend(size() + 1), __p - this->__begin_, __a);
...
vector<_Tp, _Allocator>::emplace(const_iterator __position, _Args&&... __args) {
  ...
  if (this->__end_ < this->__end_cap()) {
    if (__p == this->__end_) {
      ...
      __relocate_upward_and_insert_single(__p, this->__end_, std::move(__tmp.get()));
    }
  } else {
    allocator_type& __a = this->__alloc();
    __split_buffer<value_type, allocator_type&> __v(__recommend(size() + 1), __p - this->__begin_, __a);
...
vector<_Tp, _Allocator>::__insert_with_size(
  ...
  if (__n > 0) {
    if (__n <= this->__end_cap() - this->__end_) {
      ...
      if (__n > 0) {
        __relocate_upward_and_insert_range(__p, __old_last, __old_n, __first, __m);
      }
    } else {
      allocator_type& __a = this->__alloc();
      __split_buffer<value_type, allocator_type&> __v(__recommend(size() + __n), __p - this->__begin_, __a);
