...
template <class _Cp>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 auto begin(_Cp& __c) _NOEXCEPT_(noexcept(__c.begin())) -> decltype(__c.begin()) {
  return __c.begin();
}

template <class _Cp>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 auto begin(const _Cp& __c) _NOEXCEPT_(noexcept(__c.begin())) -> decltype(__c.begin()) {
  return __c.begin();
}

template <class _Cp>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 auto end(_Cp& __c) _NOEXCEPT_(noexcept(__c.end())) -> decltype(__c.end()) {
  return __c.end();
}

template <class _Cp>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX17 auto end(const _Cp& __c) _NOEXCEPT_(noexcept(__c.end())) -> decltype(__c.end()) {
  return __c.end();
}
