...
class _LIBCPP_MOVE_ONLY_FUNCTION_TRIVIAL_ABI move_only_function<_ReturnT(
  ...
  _LIBCPP_HIDE_FROM_ABI void __construct(_Args&&... __args) {
    ...
    constexpr bool __fits_in_buffer =
      __libcpp_is_trivially_relocatable_v<_StoredFunc> &&
...
public:
  ...
  template <class _Func>
    requires(!is_same_v<__uncvref_t<_Func>, move_only_function> && !__is_inplace_type<__uncvref_t<_Func>>::value &&
  ...
  template <class _Func>
    requires (!is_same_v<__uncvref_t<_Func>, move_only_function> &&
              !__is_inplace_type<__uncvref_t<_Func>>::value &&
              __is_callable_from<_Func>)
