...
#if defined(_LIBCPP_NONCONFORMING_OPTIONAL_MOVE_CTOR) && _LIBCPP_NONCONFORMING_OPTIONAL_MOVE_CTOR
  template <template<class, bool> class _OptionalMoveBase, bool _Bp, __enable_if_t<__libcpp_is_trivially_relocatable<_Tp>::value, int> = 0>
  _LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX20 void __construct_from(_OptionalMoveBase<_Tp, _Bp>&& __opt) {
    if (__opt.has_value()) {
      __opt.__engaged_ = false;
      std::relocate_at(std::addressof(__opt.__get()), std::addressof(this->__val_));
      this->__engaged_ = true;
    }
  }
#endif // _LIBCPP_NONCONFORMING_OPTIONAL_MOVE_CTOR
