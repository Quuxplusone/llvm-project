                              ...
                              _ForwardIt __result, true_type, _LoopWithoutCatchIgnored)
{
    ...
    if (__count != 0) {
        ...
        size_t __nbytes = (char *)std::__to_address(__last) - __firstbyte;
        std::memmove(std::__to_address(__result), __firstbyte, __nbytes);
        __result += __count;
    }
    return __result;
}
                                ...
                                _ForwardIt __result, true_type, _LoopWithoutCatchIgnored)
{
    using _Dt = typename iterator_traits<_ForwardIt>::value_type;
    if (__n != 0) {
        std::memmove(std::addressof(*__result), std::addressof(*__first), __n * sizeof (_Dt));
        __first += __n;
        __result += __n;
    }
    return {__first, __result};
