...
__remove_cvref_t<_Tp> relocate(_Tp *__source) _NOEXCEPT
{
  ...
  _LIBCPP_CLANG_DIAGNOSTIC_IGNORED("-Wcast-function-type")
  ...
  _LIBCPP_CLANG_DIAGNOSTIC_IGNORED("-Wcast-function-type")
  struct _DummyTp {
    _ALIGNAS_TYPE(_Tp) char __pad[sizeof(_Tp)];
    _DummyTp() = default; // deliberately trivial
    _DummyTp(const _DummyTp&) = default;
    _DummyTp& operator=(const _DummyTp&) = delete;
    ~_DummyTp() {} // deliberately non-trivial
  };
  auto __more_magic = (__remove_cvref_t<_Tp>(*)(const void*, size_t))+[](const void *__source, size_t __n) {
    _DummyTp __rvo; // trivially put something of the right size in x8
    std::memcpy((void*)&__rvo, (const void*)__source, __n); // do the actual memcpy into it
    return __rvo; // no-op for return
  };
  return __more_magic(__source, sizeof(_Tp));
